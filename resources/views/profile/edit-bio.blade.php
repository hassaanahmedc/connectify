<div class="w-screen fixed h-screen bg-black/80 top-0 left-0  flex justify-center items-center z-[100]"
    ...
    <section id="appointmentForm"
        class="relative bg-white max-w-screen-md bg-base-200 min-w-[350px] py-20 px-16  flex justify-center overflow-y-scroll md:w-3/4">
        {{-- close Modal Button --}}
        <svg class="swap-on fill-current absolute right-4 top-8 cursor-pointer"
            x-on:click="edit_bio=false"
        ...
        <div class="w-full">
            ...
            <form action="{{ route('profile.update', $user->id) }}"
                ...
                <div class="mt-8 ">
                    <a href="" class="px-4 py-2 text-lg mr-2 rounded bg-gray-200 hover:bg-gray-300">Cancel</a>
                    <button type="submit" class="px-8 py-2 text-lg rounded bg-lightMode-primary text-white">Save</button>
                </div>
            </form>
        </div>
    </section>
