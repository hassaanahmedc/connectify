...
<section class="w-full max-w-2xl px-4 md:w-1/2 md:px-0 lg:w-2/4">
    ...
    <div class="flex flex-col" x-data="infiniteScroll('{{ $user->followers->nextPageUrl() }}', null)">
        @forelse ($user->followers as $followerUsers)
            <x-user-card :user="$followerUsers" />
        @empty
