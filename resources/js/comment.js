...
async function createComment(event) {
    ...
    const textarea = document.querySelector(`${SELECTORS.textarea}[data-post-id="${postId}"]`);
    ...
    try {
        ...
        const commentCount = document.querySelector(`${SELECTORS.count}[data-post-id="${postId}"]`);
        commentCount.textContent = parseInt(commentCount.textContent, 10) + 1;
        textarea.value = "";
        const commentsContainer = document.querySelector(`${SELECTORS.container}[data-post-id="${postId}"]`);
...
async function deleteComment(event) {
    ...
    try {
        ...
        if (postId) {
            const count = document.querySelector(`${SELECTORS.count}[data-post-id="${postId}"]`);
